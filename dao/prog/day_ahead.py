...
class DaCalc(DaBase):
    ...
    def calc_optimum(self, _start_dt: dt.datetime | None = None,
        ...
        try:
            ...
            logging.error(ex)
        while len(b_l) < len(uur):  #addition from Cees ivm DST
          b_l.append(b_l[-1])
        ...
        if not self.heater_present:
            ...
            hp_enabled = int(self.get_state(entity_hp_enabled).state)
            if hp_enabled == 0:
              ...
              logging.info("Warmtepomp vraag actief - verbruik wordt ingepland")           
